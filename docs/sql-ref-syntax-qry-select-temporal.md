...
```sql
table_name [ FOR ] ( VERSION | SYSTEM_VERSION ) AS OF version
table_name [ FOR ] ( TIMESTAMP | SYSTEM_TIME ) AS OF timestamp_expression
```
...
* Time travel is supported only for tables that implement this capability (e.g., Delta Lake). Views, subqueries, and streaming tables do not support time travel.
...
SELECT * FROM delta_table VERSION AS OF 123;
SELECT * FROM delta_table FOR VERSION AS OF 123;

-- Version can also be a string
SELECT * FROM delta_table VERSION AS OF 'Snapshot123456789';
```
...
SELECT * FROM delta_table TIMESTAMP AS OF '2019-01-29 00:37:58';
SELECT * FROM delta_table FOR TIMESTAMP AS OF '2019-01-29 00:37:58';

-- Using a timestamp expression
SELECT * FROM delta_table TIMESTAMP AS OF current_timestamp() - INTERVAL 1 DAY;
SELECT * FROM delta_table TIMESTAMP AS OF date_sub(current_date(), 1);
```
